...
* [19/03/06] - Learning for one case
* [19/03/09] - Learning for driving case
* [19/03/11] - Appliying individual driver data
* [19/03/13] - Appliying individual driver data - q results for each case
* [19/04/23] - Appliying mpc controller instead of LQR controller
"""
    ...
    #    for sim_step in range(29000, 31500):
            ...
            if stRegCtl == 'reg on':            
                ...
                sim_rl_ctl.StoreData([control_result['acc'], control_result['accref'], control_result['reldis'],
                                      control_result['relvel'],control_result['vel'], trqRegSet, kona_power.ModBattery.SOC, acc_set, a_idm, a_mpc])
...
if swt_plot == 'on':    
    ...
    ax3.plot(sim_vehicle.DataProfile['time'], sim_vehicle.DataProfile['veh_vel_measure'], label='vel meas')
    ax3.legend()
