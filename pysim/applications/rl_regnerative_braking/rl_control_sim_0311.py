...
for driver_case in range(3):
    ...
    #        reward_array = []
        ...
        if reward_sum_array[-1] >= -223200:
            filename = 'driving_case_driver_%d_bestcase.pkl' % driver_case
            modelfilename = 'driving_case_driver_%d_bestcase.h5' % driver_case
            fcn_log_data_store([sim_vehicle, sim_algorithm, sim_idm, reward_sum_array], filename)
            agent_reg.model.save_weights(modelfilename)
            print('!!============================== result convergen ==================================!!')
    filename = 'driving_case_driver_%d.pkl' % driver_case
    modelfilename = 'driving_case_driver_%d.h5' % driver_case
    fcn_log_data_store([sim_vehicle, sim_algorithm, sim_idm, reward_sum_array], filename)
    agent_reg.model.save_weights(modelfilename)
