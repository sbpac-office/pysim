...
from rl_etc_fcn import MovAvgFilt, fcn_plot_lrn_result, fcn_set_vehicle_param, fcn_log_data_store, fcn_driving_data_arrange, fcn_epdata_arrange, TimeConstFilt
#%% import driving data
plt.close('all')
...
for drv_num in range(1,3):
    ...
    acc_set_filt = TimeConstFilt(0.1,0.01)
...
#        file_name = random.sample(file_list,1)[0]
        ...
        if drv_num == 0:
            ...
            DrivingData = fcn_driving_data_arrange(DrivingData_Tg['DrvDataGB_Case1'])
        ...
        for sim_step in range(len(DrivingData['Data_Time'])):
            ...
            if stDrvInt == 'acc on':
                ...
                if agent_reg.flag_train_state == 1:                
                    ...
                    print('file: ', file_name, 'drv_num: ', drv_num, 'it_num: ', it_num)
        ...
        if it_num >= 40:
            filename = 'driving_case_driver_it%d_drv_%d_%s.pkl' % (it_num, drv_num, file_name)
            fcn_log_data_store([sim_vehicle, sim_algorithm, sim_idm, reward_sum_array, reward_mean_array], filename)
            filename = 'factor_drivingcase_drv_%d.h5' % drv_num
            agent_reg.model.save_weights(filename)
